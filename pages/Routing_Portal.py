...
from sql import get_wiremock_data, connect_to_retool,delete_record
from wiremock import delete_wiremock_data


# Page config
st.set_page_config(
    page_title="Remove API Data - Command Center",
    layout="wide"
)
...
# Main content
st.subheader(" Wiremock Database Records")
...
try:
    with st.spinner("Loading data from database..."):
        wiremock_data = get_wiremock_data()
    
    if wiremock_data:
        st.success(f"Found {len(wiremock_data)} records in the database")
        
        # Convert to DataFrame for better display
        df = pd.DataFrame(wiremock_data)
        ...
        st.info("No records found in the wiremock database")
